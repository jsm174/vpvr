...
HRESULT Plunger::FireDispID(const DISPID dispid, DISPPARAMS * const pdispparams) {
	...
	LPOLESTR fnNames = NULL;

	if (dispid == DISPID_GameEvents_Init) {
		wstring wzName = wstring(m_wzName) + L"_Init";
		fnNames = (LPOLESTR)wzName.c_str();
	}
...
HRESULT Kicker::FireDispID(const DISPID dispid, DISPPARAMS * const pdispparams) {
	...
	else if (dispid == DISPID_HitEvents_Hit) {
		wstring wzName = wstring(m_wzName) + L"_Hit";
		fnNames = (LPOLESTR)wzName.c_str();
	}
	else if (dispid == DISPID_HitEvents_Hit) {
		wstring wzName = wstring(m_wzName) + L"_Unhit";
		fnNames = (LPOLESTR)wzName.c_str();
	}
...
HRESULT PinTable::FireDispID(const DISPID dispid, DISPPARAMS * const pdispparams) {
	...
	else if (dispid == DISPID_GameEvents_KeyDown) {
		wstring wzName = wstring(m_wzName) + L"_KeyDown";
		fnNames = (LPOLESTR)wzName.c_str();
	}
	else if (dispid == DISPID_GameEvents_KeyUp) {
		wstring wzName = wstring(m_wzName) + L"_KeyUp";
		fnNames = (LPOLESTR)wzName.c_str();
	}
...
STDMETHODIMP Collection::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	...
	WideCharToMultiByteNull(CP_ACP, 0, *rgszNames, -1, szName, MAXSTRING, nullptr, nullptr);
	if (!lstrcmpi(szName, "Item")) {
		*rgDispId = 1000;
	}
	else if (!lstrcmpi(szName, "Count")) {
		*rgDispId = 1001;
	}
	else {
...
STDMETHODIMP PinTable::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	char szName[MAXSTRING];
	WideCharToMultiByteNull(CP_ACP, 0, *rgszNames, -1, szName, MAXSTRING, nullptr, nullptr);
	if (!lstrcmpi(szName, "GlassHeight")) {
		*rgDispId = 1000;
	}
	else if (!lstrcmpi(szName, "PlayfieldMaterial")) {
		*rgDispId = 1001;
	}
	else if (!lstrcmpi(szName, "BackdropColor")) {
		*rgDispId = 1002;
	}
	else if (!lstrcmpi(szName, "SlopeMax")) {
		*rgDispId = 1003;
	}
	else if (!lstrcmpi(szName, "SlopeMin")) {
		*rgDispId = 1004;
	}
	else if (!lstrcmpi(szName, "Inclination")) {
		*rgDispId = 1005;
	}
	else if (!lstrcmpi(szName, "FieldOfView")) {
		*rgDispId = 1006;
	}
	else if (!lstrcmpi(szName, "Layback")) {
		*rgDispId = 1007;
	}
	else if (!lstrcmpi(szName, "Rotation")) {
		*rgDispId = 1008;
	}
	else if (!lstrcmpi(szName, "Scalex")) {
		*rgDispId = 1009;
	}
	else if (!lstrcmpi(szName, "Scaley")) {
		*rgDispId = 1010;
	}
	else if (!lstrcmpi(szName, "Scalez")) {
		*rgDispId = 1011;
	}
	else if (!lstrcmpi(szName, "Xlatex")) {
		*rgDispId = 1012;
	}
	else if (!lstrcmpi(szName, "Xlatey")) {
		*rgDispId = 1013;
	}
	else if (!lstrcmpi(szName, "Xlatez")) {
		*rgDispId = 1014;
	}
	else if (!lstrcmpi(szName, "Gravity")) {
		*rgDispId = 1015;
	}
	else if (!lstrcmpi(szName, "Friction")) {
		*rgDispId = 1016;
	}
	else if (!lstrcmpi(szName, "Elasticity")) {
		*rgDispId = 1017;
	}
	else if (!lstrcmpi(szName, "ElasticityFalloff")) {
		*rgDispId = 1018;
	}
	else if (!lstrcmpi(szName, "Scatter")) {
		*rgDispId = 1019;
	}
	else if (!lstrcmpi(szName, "DefaultScatter")) {
		*rgDispId = 1020;
	}
	else if (!lstrcmpi(szName, "NudgeTime")) {
		*rgDispId = 1021;
	}
	else if (!lstrcmpi(szName, "PlungerNormalize")) {
		*rgDispId = 1022;
	}
	else if (!lstrcmpi(szName, "PhysicsLoopTime")) {
		*rgDispId = 1023;
	}
	else if (!lstrcmpi(szName, "PlungerFilter")) {
		*rgDispId = 1024;
	}
	else if (!lstrcmpi(szName, "YieldTime")) {
		*rgDispId = 1025;
	}
	else if (!lstrcmpi(szName, "BallImage")) {
		*rgDispId = 1026;
	}
	else if (!lstrcmpi(szName, "BackdropImage_DT")) {
		*rgDispId = 1027;
	}
	else if (!lstrcmpi(szName, "BackdropImage_FS")) {
		*rgDispId = 1028;
	}
	else if (!lstrcmpi(szName, "BackdropImage_FSS")) {
		*rgDispId = 1029;
	}
	else if (!lstrcmpi(szName, "BackdropImageApplyNightDay")) {
		*rgDispId = 1030;
	}
	else if (!lstrcmpi(szName, "ColorGradeImage")) {
		*rgDispId = 1031;
	}
	else if (!lstrcmpi(szName, "Width")) {
		*rgDispId = 1032;
	}
	else if (!lstrcmpi(szName, "Height")) {
		*rgDispId = 1033;
	}
	else if (!lstrcmpi(szName, "MaxSeparation")) {
		*rgDispId = 1034;
	}
	else if (!lstrcmpi(szName, "ZPD")) {
		*rgDispId = 1035;
	}
	else if (!lstrcmpi(szName, "Offset")) {
		*rgDispId = 1036;
	}
	else if (!lstrcmpi(szName, "GlobalStereo3D")) {
		*rgDispId = 1037;
	}
	else if (!lstrcmpi(szName, "BallDecalMode")) {
		*rgDispId = 1038;
	}
	else if (!lstrcmpi(szName, "Image")) {
		*rgDispId = 1039;
	}
	else if (!lstrcmpi(szName, "FileName")) {
		*rgDispId = 1040;
	}
	else if (!lstrcmpi(szName, "Name")) {
		*rgDispId = 1041;
	}
	else if (!lstrcmpi(szName, "EnableAntialiasing")) {
		*rgDispId = 1042;
	}
	else if (!lstrcmpi(szName, "EnableAO")) {
		*rgDispId = 1043;
	}
	else if (!lstrcmpi(szName, "EnableFXAA")) {
		*rgDispId = 1044;
	}
	else if (!lstrcmpi(szName, "EnableSSR")) {
		*rgDispId = 1045;
	}
	else if (!lstrcmpi(szName, "BloomStrength")) {
		*rgDispId = 1046;
	}
	else if (!lstrcmpi(szName, "BallFrontDecal")) {
		*rgDispId = 1047;
	}
	else if (!lstrcmpi(szName, "OverridePhysics")) {
		*rgDispId = 1048;
	}
	else if (!lstrcmpi(szName, "OverridePhysicsFlippers")) {
		*rgDispId = 1049;
	}
	else if (!lstrcmpi(szName, "EnableEMReels")) {
		*rgDispId = 1050;
	}
	else if (!lstrcmpi(szName, "EnableDecals")) {
		*rgDispId = 1051;
	}
	else if (!lstrcmpi(szName, "ShowDT")) {
		*rgDispId = 1052;
	}
	else if (!lstrcmpi(szName, "ShowFSS")) {
		*rgDispId = 1053;
	}
	else if (!lstrcmpi(szName, "ReflectElementsOnPlayfield")) {
		*rgDispId = 1054;
	}
	else if (!lstrcmpi(szName, "EnvironmentImage")) {
		*rgDispId = 1055;
	}
	else if (!lstrcmpi(szName, "BackglassMode")) {
		*rgDispId = 1056;
	}
	else if (!lstrcmpi(szName, "Accelerometer")) {
		*rgDispId = 1057;
	}
	else if (!lstrcmpi(szName, "AccelNormalMount")) {
		*rgDispId = 1058;
	}
	else if (!lstrcmpi(szName, "AccelerometerAngle")) {
		*rgDispId = 1059;
	}
	else if (!lstrcmpi(szName, "GlobalDifficulty")) {
		*rgDispId = 1060;
	}
	else if (!lstrcmpi(szName, "TableHeight")) {
		*rgDispId = 1061;
	}
	else if (!lstrcmpi(szName, "DeadZone")) {
		*rgDispId = 1062;
	}
	else if (!lstrcmpi(szName, "LightAmbient")) {
		*rgDispId = 1063;
	}
	else if (!lstrcmpi(szName, "Light0Emission")) {
		*rgDispId = 1064;
	}
	else if (!lstrcmpi(szName, "LightHeight")) {
		*rgDispId = 1065;
	}
	else if (!lstrcmpi(szName, "LightRange")) {
		*rgDispId = 1066;
	}
	else if (!lstrcmpi(szName, "EnvironmentEmissionScale")) {
		*rgDispId = 1067;
	}
	else if (!lstrcmpi(szName, "LightEmissionScale")) {
		*rgDispId = 1068;
	}
	else if (!lstrcmpi(szName, "AOScale")) {
		*rgDispId = 1069;
	}
	else if (!lstrcmpi(szName, "SSRScale")) {
		*rgDispId = 1070;
	}
	else if (!lstrcmpi(szName, "TableSoundVolume")) {
		*rgDispId = 1071;
	}
	else if (!lstrcmpi(szName, "TableMusicVolume")) {
		*rgDispId = 1072;
	}
	else if (!lstrcmpi(szName, "TableAdaptiveVSync")) {
		*rgDispId = 1073;
	}
	else if (!lstrcmpi(szName, "BallReflection")) {
		*rgDispId = 1074;
	}
	else if (!lstrcmpi(szName, "PlayfieldReflectionStrength")) {
		*rgDispId = 1075;
	}
	else if (!lstrcmpi(szName, "BallTrail")) {
		*rgDispId = 1076;
	}
	else if (!lstrcmpi(szName, "TrailStrength")) {
		*rgDispId = 1077;
	}
	else if (!lstrcmpi(szName, "BallPlayfieldReflectionScale")) {
		*rgDispId = 1078;
	}
	else if (!lstrcmpi(szName, "DefaultBulbIntensityScale")) {
		*rgDispId = 1079;
	}
	else if (!lstrcmpi(szName, "DetailLevel")) {
		*rgDispId = 1080;
	}
	else if (!lstrcmpi(szName, "NightDay")) {
		*rgDispId = 1081;
	}
	else if (!lstrcmpi(szName, "GlobalAlphaAcc")) {
		*rgDispId = 1082;
	}
	else if (!lstrcmpi(szName, "GlobalDayNight")) {
		*rgDispId = 1083;
	}
	else if (!lstrcmpi(szName, "Version")) {
		*rgDispId = 1084;
	}
	else if (!lstrcmpi(szName, "VersionMajor")) {
		*rgDispId = 1085;
	}
	else if (!lstrcmpi(szName, "VersionMinor")) {
		*rgDispId = 1086;
	}
	else if (!lstrcmpi(szName, "VersionRevision")) {
		*rgDispId = 1087;
	}
	else if (!lstrcmpi(szName, "VPBuildVersion")) {
		*rgDispId = 1088;
	}
	else {
		return DISP_E_UNKNOWNNAME;
...
STDMETHODIMP ScriptGlobalTable::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	char szName[MAXSTRING];
	WideCharToMultiByteNull(CP_ACP, 0, *rgszNames, -1, szName, MAXSTRING, nullptr, nullptr);
	if (!lstrcmpi(szName, "PlaySound")) {
		*rgDispId = 1000;
	}
	else if (!lstrcmpi(szName, "LeftFlipperKey")) {
		*rgDispId = 1001;
	}
	else if (!lstrcmpi(szName, "RightFlipperKey")) {
		*rgDispId = 1002;
	}
	else if (!lstrcmpi(szName, "LeftTiltKey")) {
		*rgDispId = 1003;
	}
	else if (!lstrcmpi(szName, "RightTiltKey")) {
		*rgDispId = 1004;
	}
	else if (!lstrcmpi(szName, "CenterTiltKey")) {
		*rgDispId = 1005;
	}
	else if (!lstrcmpi(szName, "PlungerKey")) {
		*rgDispId = 1006;
	}
	else if (!lstrcmpi(szName, "PlayMusic")) {
		*rgDispId = 1007;
	}
	else if (!lstrcmpi(szName, "MusicVolume")) {
		*rgDispId = 1008;
	}
	else if (!lstrcmpi(szName, "EndMusic")) {
		*rgDispId = 1009;
	}
	else if (!lstrcmpi(szName, "StartGameKey")) {
		*rgDispId = 1010;
	}
	else if (!lstrcmpi(szName, "UserDirectory")) {
		*rgDispId = 1011;
	}
	else if (!lstrcmpi(szName, "GetPlayerHWnd")) {
		*rgDispId = 1012;
	}
	else if (!lstrcmpi(szName, "StopSound")) {
		*rgDispId = 1013;
	}
	else if (!lstrcmpi(szName, "SaveValue")) {
		*rgDispId = 1014;
	}
	else if (!lstrcmpi(szName, "LoadValue")) {
		*rgDispId = 1015;
	}
	else if (!lstrcmpi(szName, "ActiveBall")) {
		*rgDispId = 1016;
	}
	else if (!lstrcmpi(szName, "AddCreditKey")) {
		*rgDispId = 1017;
	}
	else if (!lstrcmpi(szName, "AddCreditKey2")) {
		*rgDispId = 1018;
	}
	else if (!lstrcmpi(szName, "GameTime")) {
		*rgDispId = 1019;
	}
	else if (!lstrcmpi(szName, "SystemTime")) {
		*rgDispId = 1020;
	}
	else if (!lstrcmpi(szName, "GetCustomParam")) {
		*rgDispId = 1021;
	}
	else if (!lstrcmpi(szName, "GetTextFile")) {
		*rgDispId = 1022;
	}
	else if (!lstrcmpi(szName, "BeginModal")) {
		*rgDispId = 1023;
	}
	else if (!lstrcmpi(szName, "EndModal")) {
		*rgDispId = 1024;
	}
	else if (!lstrcmpi(szName, "Nudge")) {
		*rgDispId = 1025;
	}
	else if (!lstrcmpi(szName, "NudgeGetCalibration")) {
		*rgDispId = 1026;
	}
	else if (!lstrcmpi(szName, "NudgeSetCalibration")) {
		*rgDispId = 1027;
	}
	else if (!lstrcmpi(szName, "NudgeSensorStatus")) {
		*rgDispId = 1028;
	}
	else if (!lstrcmpi(szName, "NudgeTiltStatus")) {
		*rgDispId = 1029;
	}
	else if (!lstrcmpi(szName, "MechanicalTilt")) {
		*rgDispId = 1030;
	}
	else if (!lstrcmpi(szName, "LeftMagnaSave")) {
		*rgDispId = 1031;
	}
	else if (!lstrcmpi(szName, "RightMagnaSave")) {
		*rgDispId = 1032;
	}
	else if (!lstrcmpi(szName, "ExitGame")) {
		*rgDispId = 1033;
	}
	else if (!lstrcmpi(szName, "LockbarKey")) {
		*rgDispId = 1034;
	}
	else if (!lstrcmpi(szName, "FireKnocker")) {
		*rgDispId = 1035;
	}
	else if (!lstrcmpi(szName, "QuitPlayer")) {
		*rgDispId = 1036;
	}
	else if (!lstrcmpi(szName, "ShowDT")) {
		*rgDispId = 1037;
	...
	else if (!lstrcmpi(szName, "GetBalls")) {
		*rgDispId = 1040;
	}
	else if (!lstrcmpi(szName, "GetElements")) {
		*rgDispId = 1041;
	}
	else if (!lstrcmpi(szName, "GetElementByName")) {
		*rgDispId = 1042;
	}
	else if (!lstrcmpi(szName, "ActiveTable")) {
		*rgDispId = 1043;
	}
	else if (!lstrcmpi(szName, "DMDWidth")) {
		*rgDispId = 1044;
	}
	else if (!lstrcmpi(szName, "DMDHeight")) {
		*rgDispId = 1045;
	}
	else if (!lstrcmpi(szName, "DMDPixels")) {
		*rgDispId = 1046;
	}
	else if (!lstrcmpi(szName, "DMDColoredPixels")) {
		*rgDispId = 1047;
	}
	else if (!lstrcmpi(szName, "Version")) {
		*rgDispId = 1048;
	...
	else if (!lstrcmpi(szName, "GetSerialDevices")) {
		*rgDispId = 1053;
	}
	else if (!lstrcmpi(szName, "OpenSerial")) {
		*rgDispId = 1054;
	}
	else if (!lstrcmpi(szName, "CloseSerial")) {
		*rgDispId = 1055;
	}
	else if (!lstrcmpi(szName, "FlushSerial")) {
		*rgDispId = 1056;
	}
	else if (!lstrcmpi(szName, "SetupSerial")) {
		*rgDispId = 1057;
	}
	else if (!lstrcmpi(szName, "ReadSerial")) {
		*rgDispId = 1058;
	}
	else if (!lstrcmpi(szName, "WriteSerial")) {
		*rgDispId = 1059;
	}
	else if (!lstrcmpi(szName, "RenderingMode")) {
		*rgDispId = 1060;
	}
	else if (!lstrcmpi(szName, "UpdateMaterial")) {
		*rgDispId = 1061;
	}
	else if (!lstrcmpi(szName, "GetMaterial")) {
		*rgDispId = 1062;
	}
	else if (!lstrcmpi(szName, "UpdateMaterialPhysics")) {
		*rgDispId = 1063;
	}
	else if (!lstrcmpi(szName, "GetMaterialPhysics")) {
		*rgDispId = 1064;
	}
	else if (!lstrcmpi(szName, "MaterialColor")) {
		*rgDispId = 1065;
	}
	else if (!lstrcmpi(szName, "WindowWidth")) {
		*rgDispId = 1066;
	}
	else if (!lstrcmpi(szName, "WindowHeight")) {
		*rgDispId = 1067;
	}
	else {
		return DISP_E_UNKNOWNNAME;
...
STDMETHODIMP ScriptGlobalTable::Invoke(DISPID dispIdMember, REFIID /*riid*/, LCID lcid, WORD wFlags, DISPPARAMS* pDispParams, VARIANT* pVarResult, EXCEPINFO* pExcepInfo, UINT* puArgErr) {
	switch(dispIdMember) {
		...
		case 1040: {
			...
			V_VT(pVarResult) = VT_DISPATCH;
			return GetBalls((SAFEARRAY**)&V_DISPATCH(pVarResult));
		}
		...
		case 1041: {
			...
			V_VT(pVarResult) = VT_DISPATCH;
			return GetElements((SAFEARRAY**)&V_DISPATCH(pVarResult));
		}
...
STDMETHODIMP Surface::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	char szName[MAXSTRING];
	WideCharToMultiByteNull(CP_ACP, 0, *rgszNames, -1, szName, MAXSTRING, nullptr, nullptr);
	if (!lstrcmpi(szName, "Name")) {
		*rgDispId = 1000;
	}
	else if (!lstrcmpi(szName, "TimerEnabled")) {
		*rgDispId = 1001;
	}
	else if (!lstrcmpi(szName, "TimerInterval")) {
		*rgDispId = 1002;
	}
	else if (!lstrcmpi(szName, "HasHitEvent")) {
		*rgDispId = 1003;
	}
	else if (!lstrcmpi(szName, "Threshold")) {
		*rgDispId = 1004;
	}
	else if (!lstrcmpi(szName, "Image")) {
		*rgDispId = 1005;
	}
	else if (!lstrcmpi(szName, "SideMaterial")) {
		*rgDispId = 1006;
	}
	else if (!lstrcmpi(szName, "ImageAlignment")) {
		*rgDispId = 1007;
	}
	else if (!lstrcmpi(szName, "HeightBottom")) {
		*rgDispId = 1008;
	}
	else if (!lstrcmpi(szName, "HeightTop")) {
		*rgDispId = 1009;
	}
	else if (!lstrcmpi(szName, "TopMaterial")) {
		*rgDispId = 1010;
	}
	else if (!lstrcmpi(szName, "CanDrop")) {
		*rgDispId = 1011;
	}
	else if (!lstrcmpi(szName, "Collidable")) {
		*rgDispId = 1012;
	}
	else if (!lstrcmpi(szName, "IsDropped")) {
		*rgDispId = 1013;
	}
	else if (!lstrcmpi(szName, "DisplayTexture")) {
		*rgDispId = 1014;
	}
	else if (!lstrcmpi(szName, "SlingshotStrength")) {
		*rgDispId = 1015;
	}
	else if (!lstrcmpi(szName, "Elasticity")) {
		*rgDispId = 1016;
	...
	else if (!lstrcmpi(szName, "SideImage")) {
		*rgDispId = 1018;
	}
	else if (!lstrcmpi(szName, "Visible")) {
		*rgDispId = 1019;
	}
	else if (!lstrcmpi(szName, "Disabled")) {
		*rgDispId = 1020;
	}
	else if (!lstrcmpi(szName, "SideVisible")) {
		*rgDispId = 1021;
	}
	else if (!lstrcmpi(szName, "UserValue")) {
		*rgDispId = 1022;
	}
	else if (!lstrcmpi(szName, "SlingshotMaterial")) {
		*rgDispId = 1023;
	}
	else if (!lstrcmpi(szName, "SlingshotThreshold")) {
		*rgDispId = 1024;
	}
	else if (!lstrcmpi(szName, "SlingshotAnimation")) {
		*rgDispId = 1025;
	}
	else if (!lstrcmpi(szName, "FlipbookAnimation")) {
		*rgDispId = 1026;
	}
	else if (!lstrcmpi(szName, "IsBottomSolid")) {
		*rgDispId = 1027;
	}
	else if (!lstrcmpi(szName, "DisableLighting")) {
		*rgDispId = 1028;
	}
	else if (!lstrcmpi(szName, "BlendDisableLighting")) {
		*rgDispId = 1029;
	}
	else if (!lstrcmpi(szName, "BlendDisableLightingFromBelow")) {
		*rgDispId = 1030;
	}
	else if (!lstrcmpi(szName, "Friction")) {
		*rgDispId = 1031;
	...
	else if (!lstrcmpi(szName, "ReflectionEnabled")) {
		*rgDispId = 1033;
	}
	else if (!lstrcmpi(szName, "PlaySlingshotHit")) {
		*rgDispId = 1034;
	}
	else if (!lstrcmpi(szName, "PhysicsMaterial")) {
		*rgDispId = 1035;
	}
	else if (!lstrcmpi(szName, "OverwritePhysics")) {
		*rgDispId = 1036;
	}
	else {
		return DISP_E_UNKNOWNNAME;
...
STDMETHODIMP DragPoint::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	char szName[MAXSTRING];
	WideCharToMultiByteNull(CP_ACP, 0, *rgszNames, -1, szName, MAXSTRING, nullptr, nullptr);
	if (!lstrcmpi(szName, "X")) {
		*rgDispId = 1000;
	}
	else if (!lstrcmpi(szName, "Y")) {
		*rgDispId = 1001;
	}
	else if (!lstrcmpi(szName, "Z")) {
		*rgDispId = 1002;
	}
	else if (!lstrcmpi(szName, "CalcHeight")) {
		*rgDispId = 1003;
	}
	else if (!lstrcmpi(szName, "Smooth")) {
		*rgDispId = 1004;
	}
	else if (!lstrcmpi(szName, "IsAutoTextureCoordinate")) {
		*rgDispId = 1005;
	}
	else if (!lstrcmpi(szName, "TextureCoordinateU")) {
		*rgDispId = 1006;
	}
	else {
		return DISP_E_UNKNOWNNAME;
...
STDMETHODIMP Flipper::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	char szName[MAXSTRING];
	WideCharToMultiByteNull(CP_ACP, 0, *rgszNames, -1, szName, MAXSTRING, nullptr, nullptr);
	if (!lstrcmpi(szName, "BaseRadius")) {
		*rgDispId = 1000;
	}
	else if (!lstrcmpi(szName, "EndRadius")) {
		*rgDispId = 1001;
	}
	else if (!lstrcmpi(szName, "Length")) {
		*rgDispId = 1002;
	}
	else if (!lstrcmpi(szName, "StartAngle")) {
		*rgDispId = 1003;
	}
	else if (!lstrcmpi(szName, "RotateToEnd")) {
		*rgDispId = 1004;
	}
	else if (!lstrcmpi(szName, "RotateToStart")) {
		*rgDispId = 1005;
	}
	else if (!lstrcmpi(szName, "EndAngle")) {
		*rgDispId = 1006;
	}
	else if (!lstrcmpi(szName, "TimerEnabled")) {
		*rgDispId = 1007;
	...
	else if (!lstrcmpi(szName, "CurrentAngle")) {
		*rgDispId = 1009;
	}
	else if (!lstrcmpi(szName, "X")) {
		*rgDispId = 1010;
	}
	else if (!lstrcmpi(szName, "Y")) {
		*rgDispId = 1011;
	}
	else if (!lstrcmpi(szName, "Material")) {
		*rgDispId = 1012;
	}
	else if (!lstrcmpi(szName, "Mass")) {
		*rgDispId = 1013;
	}
	else if (!lstrcmpi(szName, "OverridePhysics")) {
		*rgDispId = 1014;
	}
	else if (!lstrcmpi(szName, "RubberMaterial")) {
		*rgDispId = 1015;
	}
	else if (!lstrcmpi(szName, "RubberThickness")) {
		*rgDispId = 1016;
	}
	else if (!lstrcmpi(szName, "Strength")) {
		*rgDispId = 1017;
	}
	else if (!lstrcmpi(szName, "Visible")) {
		*rgDispId = 1018;
	}
	else if (!lstrcmpi(szName, "Enabled")) {
		*rgDispId = 1019;
	}
	else if (!lstrcmpi(szName, "Elasticity")) {
		*rgDispId = 1020;
	...
	else if (!lstrcmpi(szName, "Return")) {
		*rgDispId = 1022;
	}
	else if (!lstrcmpi(szName, "RubberHeight")) {
		*rgDispId = 1023;
	}
	else if (!lstrcmpi(szName, "RubberWidth")) {
		*rgDispId = 1024;
	}
	else if (!lstrcmpi(szName, "Friction")) {
		*rgDispId = 1025;
	}
	else if (!lstrcmpi(szName, "RampUp")) {
		*rgDispId = 1026;
	}
	else if (!lstrcmpi(szName, "ElasticityFalloff")) {
		*rgDispId = 1027;
	}
	else if (!lstrcmpi(szName, "Surface")) {
		*rgDispId = 1028;
	}
	else if (!lstrcmpi(szName, "Name")) {
		*rgDispId = 1029;
	...
	else if (!lstrcmpi(szName, "EOSTorque")) {
		*rgDispId = 1032;
	}
	else if (!lstrcmpi(szName, "EOSTorqueAngle")) {
		*rgDispId = 1033;
	}
	else if (!lstrcmpi(szName, "FlipperRadiusMin")) {
		*rgDispId = 1034;
	}
	else if (!lstrcmpi(szName, "Image")) {
		*rgDispId = 1035;
...
STDMETHODIMP Timer::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	char szName[MAXSTRING];
	WideCharToMultiByteNull(CP_ACP, 0, *rgszNames, -1, szName, MAXSTRING, nullptr, nullptr);
	if (!lstrcmpi(szName, "Enabled")) {
		*rgDispId = 1000;
	}
	else if (!lstrcmpi(szName, "Interval")) {
		*rgDispId = 1001;
	}
	else if (!lstrcmpi(szName, "Name")) {
		*rgDispId = 1002;
...
STDMETHODIMP Plunger::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	char szName[MAXSTRING];
	WideCharToMultiByteNull(CP_ACP, 0, *rgszNames, -1, szName, MAXSTRING, nullptr, nullptr);
	if (!lstrcmpi(szName, "PullBack")) {
		*rgDispId = 1000;
	}
	else if (!lstrcmpi(szName, "Fire")) {
		*rgDispId = 1001;
	}
	else if (!lstrcmpi(szName, "PullSpeed")) {
		*rgDispId = 1002;
	}
	else if (!lstrcmpi(szName, "FireSpeed")) {
		*rgDispId = 1003;
	}
	else if (!lstrcmpi(szName, "CreateBall")) {
		*rgDispId = 1004;
	}
	else if (!lstrcmpi(szName, "Position")) {
		*rgDispId = 1005;
	}
	else if (!lstrcmpi(szName, "PullBackandRetract")) {
		*rgDispId = 1006;
	}
	else if (!lstrcmpi(szName, "MotionDevice")) {
		*rgDispId = 1007;
	}
	else if (!lstrcmpi(szName, "TimerEnabled")) {
		*rgDispId = 1008;
	...
	else if (!lstrcmpi(szName, "ZAdjust")) {
		*rgDispId = 1013;
	}
	else if (!lstrcmpi(szName, "Surface")) {
		*rgDispId = 1014;
	...
	else if (!lstrcmpi(szName, "AnimFrames")) {
		*rgDispId = 1020;
	}
	else if (!lstrcmpi(szName, "TipShape")) {
		*rgDispId = 1021;
	}
	else if (!lstrcmpi(szName, "RodDiam")) {
		*rgDispId = 1022;
	}
	else if (!lstrcmpi(szName, "RingGap")) {
		*rgDispId = 1023;
	}
	else if (!lstrcmpi(szName, "RingDiam")) {
		*rgDispId = 1024;
	}
	else if (!lstrcmpi(szName, "RingWidth")) {
		*rgDispId = 1025;
	}
	else if (!lstrcmpi(szName, "SpringDiam")) {
		*rgDispId = 1026;
	}
	else if (!lstrcmpi(szName, "SpringGauge")) {
		*rgDispId = 1027;
	}
	else if (!lstrcmpi(szName, "SpringLoops")) {
		*rgDispId = 1028;
	}
	else if (!lstrcmpi(szName, "SpringEndLoops")) {
		*rgDispId = 1029;
	}
	else if (!lstrcmpi(szName, "MechPlunger")) {
		*rgDispId = 1030;
	}
	else if (!lstrcmpi(szName, "AutoPlunger")) {
		*rgDispId = 1031;
	}
	else if (!lstrcmpi(szName, "Visible")) {
		*rgDispId = 1032;
	}
	else if (!lstrcmpi(szName, "MechStrength")) {
		*rgDispId = 1033;
	}
	else if (!lstrcmpi(szName, "ParkPosition")) {
		*rgDispId = 1034;
	}
	else if (!lstrcmpi(szName, "Stroke")) {
		*rgDispId = 1035;
	}
	else if (!lstrcmpi(szName, "ScatterVelocity")) {
		*rgDispId = 1036;
	}
	else if (!lstrcmpi(szName, "MomentumXfer")) {
		*rgDispId = 1037;
	}
	else if (!lstrcmpi(szName, "ReflectionEnabled")) {
		*rgDispId = 1038;
...
STDMETHODIMP Textbox::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	char szName[MAXSTRING];
	WideCharToMultiByteNull(CP_ACP, 0, *rgszNames, -1, szName, MAXSTRING, nullptr, nullptr);
	if (!lstrcmpi(szName, "BackColor")) {
		*rgDispId = 1000;
	}
	else if (!lstrcmpi(szName, "FontColor")) {
		*rgDispId = 1001;
	}
	else if (!lstrcmpi(szName, "Text")) {
		*rgDispId = 1002;
	}
	else if (!lstrcmpi(szName, "Font")) {
		*rgDispId = 1003;
	}
	else if (!lstrcmpi(szName, "putref_Font")) {
		*rgDispId = 1004;
	}
	else if (!lstrcmpi(szName, "Width")) {
		*rgDispId = 1005;
	...
	else if (!lstrcmpi(szName, "Alignment")) {
		*rgDispId = 1007;
	}
	else if (!lstrcmpi(szName, "IsTransparent")) {
		*rgDispId = 1008;
	}
	else if (!lstrcmpi(szName, "DMD")) {
		*rgDispId = 1009;
	}
	else if (!lstrcmpi(szName, "TimerEnabled")) {
		*rgDispId = 1010;
	...
	else if (!lstrcmpi(szName, "IntensityScale")) {
		*rgDispId = 1014;
	}
	else if (!lstrcmpi(szName, "Name")) {
		*rgDispId = 1015;
...
STDMETHODIMP Bumper::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	...
	else if (!lstrcmpi(szName, "BaseMaterial")) {
		*rgDispId = 1003;
	}
	else if (!lstrcmpi(szName, "SkirtMaterial")) {
		*rgDispId = 1004;
	}
	else if (!lstrcmpi(szName, "UserValue")) {
		*rgDispId = 1005;
	...
	else if (!lstrcmpi(szName, "CapMaterial")) {
		*rgDispId = 1011;
	}
	else if (!lstrcmpi(szName, "RingMaterial")) {
		*rgDispId = 1012;
	}
	else if (!lstrcmpi(szName, "HeightScale")) {
		*rgDispId = 1013;
	}
	else if (!lstrcmpi(szName, "Orientation")) {
		*rgDispId = 1014;
	}
	else if (!lstrcmpi(szName, "RingSpeed")) {
		*rgDispId = 1015;
	}
	else if (!lstrcmpi(szName, "RingDropOffset")) {
		*rgDispId = 1016;
	}
	else if (!lstrcmpi(szName, "Name")) {
		*rgDispId = 1017;
	...
	else if (!lstrcmpi(szName, "CapVisible")) {
		*rgDispId = 1019;
	}
	else if (!lstrcmpi(szName, "BaseVisible")) {
		*rgDispId = 1020;
	}
	else if (!lstrcmpi(szName, "RingVisible")) {
		*rgDispId = 1021;
	}
	else if (!lstrcmpi(szName, "SkirtVisible")) {
		*rgDispId = 1022;
	}
	else if (!lstrcmpi(szName, "Collidable")) {
		*rgDispId = 1023;
	...
	else if (!lstrcmpi(szName, "EnableSkirtAnimation")) {
		*rgDispId = 1026;
	}
	else if (!lstrcmpi(szName, "PlayHit")) {
		*rgDispId = 1027;
	}
	else {
		return DISP_E_UNKNOWNNAME;
...
STDMETHODIMP Trigger::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	...
	else if (!lstrcmpi(szName, "TriggerShape")) {
		*rgDispId = 1007;
	}
	else if (!lstrcmpi(szName, "Surface")) {
		*rgDispId = 1008;
	...
	else if (!lstrcmpi(szName, "BallCntOver")) {
		*rgDispId = 1011;
	}
	else if (!lstrcmpi(szName, "DestroyBall")) {
		*rgDispId = 1012;
	}
	else if (!lstrcmpi(szName, "HitHeight")) {
		*rgDispId = 1013;
	}
	else if (!lstrcmpi(szName, "Material")) {
		*rgDispId = 1014;
	...
	else if (!lstrcmpi(szName, "WireThickness")) {
		*rgDispId = 1016;
	}
	else if (!lstrcmpi(szName, "AnimSpeed")) {
		*rgDispId = 1017;
	}
	else if (!lstrcmpi(szName, "ReflectionEnabled")) {
		*rgDispId = 1018;
...
STDMETHODIMP Light::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	char szName[MAXSTRING];
	WideCharToMultiByteNull(CP_ACP, 0, *rgszNames, -1, szName, MAXSTRING, nullptr, nullptr);
	if (!lstrcmpi(szName, "Falloff")) {
		*rgDispId = 1000;
	}
	else if (!lstrcmpi(szName, "FalloffPower")) {
		*rgDispId = 1001;
	}
	else if (!lstrcmpi(szName, "State")) {
		*rgDispId = 1002;
	}
	else if (!lstrcmpi(szName, "GetInPlayState")) {
		*rgDispId = 1003;
	}
	else if (!lstrcmpi(szName, "GetInPlayStateBool")) {
		*rgDispId = 1004;
	}
	else if (!lstrcmpi(szName, "GetInPlayIntensity")) {
		*rgDispId = 1005;
	}
	else if (!lstrcmpi(szName, "Color")) {
		*rgDispId = 1006;
	}
	else if (!lstrcmpi(szName, "ColorFull")) {
		*rgDispId = 1007;
	}
	else if (!lstrcmpi(szName, "TimerEnabled")) {
		*rgDispId = 1008;
	...
	else if (!lstrcmpi(szName, "BlinkPattern")) {
		*rgDispId = 1012;
	}
	else if (!lstrcmpi(szName, "BlinkInterval")) {
		*rgDispId = 1013;
	}
	else if (!lstrcmpi(szName, "Duration")) {
		*rgDispId = 1014;
	}
	else if (!lstrcmpi(szName, "Intensity")) {
		*rgDispId = 1015;
	}
	else if (!lstrcmpi(szName, "TransmissionScale")) {
		*rgDispId = 1016;
	}
	else if (!lstrcmpi(szName, "IntensityScale")) {
		*rgDispId = 1017;
	...
	else if (!lstrcmpi(szName, "ImageMode")) {
		*rgDispId = 1022;
	}
	else if (!lstrcmpi(szName, "DepthBias")) {
		*rgDispId = 1023;
	}
	else if (!lstrcmpi(szName, "FadeSpeedUp")) {
		*rgDispId = 1024;
	}
	else if (!lstrcmpi(szName, "FadeSpeedDown")) {
		*rgDispId = 1025;
	}
	else if (!lstrcmpi(szName, "Bulb")) {
		*rgDispId = 1026;
	}
	else if (!lstrcmpi(szName, "ShowBulbMesh")) {
		*rgDispId = 1027;
	}
	else if (!lstrcmpi(szName, "StaticBulbMesh")) {
		*rgDispId = 1028;
	}
	else if (!lstrcmpi(szName, "ShowReflectionOnBall")) {
		*rgDispId = 1029;
	}
	else if (!lstrcmpi(szName, "ScaleBulbMesh")) {
		*rgDispId = 1030;
	}
	else if (!lstrcmpi(szName, "BulbModulateVsAdd")) {
		*rgDispId = 1031;
	}
	else if (!lstrcmpi(szName, "BulbHaloHeight")) {
		*rgDispId = 1032;
	}
	else if (!lstrcmpi(szName, "Visible")) {
		*rgDispId = 1033;
...
STDMETHODIMP Kicker::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	...
	else if (!lstrcmpi(szName, "CreateSizedBall")) {
		*rgDispId = 1002;
	}
	else if (!lstrcmpi(szName, "CreateSizedBallWithMass")) {
		*rgDispId = 1003;
	}
	else if (!lstrcmpi(szName, "DestroyBall")) {
		*rgDispId = 1004;
	...
	else if (!lstrcmpi(szName, "Kick")) {
		*rgDispId = 1007;
	}
	else if (!lstrcmpi(szName, "KickXYZ")) {
		*rgDispId = 1008;
	}
	else if (!lstrcmpi(szName, "KickZ")) {
		*rgDispId = 1009;
	}
	else if (!lstrcmpi(szName, "X")) {
		*rgDispId = 1010;
	...
	else if (!lstrcmpi(szName, "DrawStyle")) {
		*rgDispId = 1013;
	}
	else if (!lstrcmpi(szName, "Material")) {
		*rgDispId = 1014;
	...
	else if (!lstrcmpi(szName, "HitAccuracy")) {
		*rgDispId = 1019;
	}
	else if (!lstrcmpi(szName, "HitHeight")) {
		*rgDispId = 1020;
	...
	else if (!lstrcmpi(szName, "Radius")) {
		*rgDispId = 1022;
	}
	else if (!lstrcmpi(szName, "FallThrough")) {
		*rgDispId = 1023;
	}
	else if (!lstrcmpi(szName, "Legacy")) {
		*rgDispId = 1024;
	}
	else if (!lstrcmpi(szName, "LastCapturedBall")) {
		*rgDispId = 1025;
	}
	else {
		return DISP_E_UNKNOWNNAME;
...
STDMETHODIMP Decal::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	...
	else if (!lstrcmpi(szName, "SizingType")) {
		*rgDispId = 1008;
	}
	else if (!lstrcmpi(szName, "FontColor")) {
		*rgDispId = 1009;
	...
	else if (!lstrcmpi(szName, "HasVerticalText")) {
		*rgDispId = 1013;
	}
	else if (!lstrcmpi(szName, "Surface")) {
		*rgDispId = 1014;
...
STDMETHODIMP Primitive::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	char szName[MAXSTRING];
	WideCharToMultiByteNull(CP_ACP, 0, *rgszNames, -1, szName, MAXSTRING, nullptr, nullptr);
	if (!lstrcmpi(szName, "PlayAnim")) {
		*rgDispId = 1000;
	}
	else if (!lstrcmpi(szName, "PlayAnimEndless")) {
		*rgDispId = 1001;
	}
	else if (!lstrcmpi(szName, "StopAnim")) {
		*rgDispId = 1002;
	}
	else if (!lstrcmpi(szName, "ShowFrame")) {
		*rgDispId = 1003;
	}
	else if (!lstrcmpi(szName, "ContinueAnim")) {
		*rgDispId = 1004;
	}
	else if (!lstrcmpi(szName, "DisplayTexture")) {
		*rgDispId = 1005;
	}
	else if (!lstrcmpi(szName, "Sides")) {
		*rgDispId = 1006;
	}
	else if (!lstrcmpi(szName, "Visible")) {
		*rgDispId = 1007;
	...
	else if (!lstrcmpi(szName, "SideColor")) {
		*rgDispId = 1009;
	}
	else if (!lstrcmpi(szName, "DrawTexturesInside")) {
		*rgDispId = 1010;
	}
	else if (!lstrcmpi(szName, "UserValue")) {
		*rgDispId = 1011;
	...
	else if (!lstrcmpi(szName, "NormalMap")) {
		*rgDispId = 1013;
	}
	else if (!lstrcmpi(szName, "X")) {
		*rgDispId = 1014;
	...
	else if (!lstrcmpi(szName, "Size_X")) {
		*rgDispId = 1017;
	}
	else if (!lstrcmpi(szName, "Size_Y")) {
		*rgDispId = 1018;
	}
	else if (!lstrcmpi(szName, "Size_Z")) {
		*rgDispId = 1019;
	}
	else if (!lstrcmpi(szName, "RotAndTra0")) {
		*rgDispId = 1020;
	}
	else if (!lstrcmpi(szName, "RotX")) {
		*rgDispId = 1021;
	}
	else if (!lstrcmpi(szName, "RotAndTra1")) {
		*rgDispId = 1022;
	}
	else if (!lstrcmpi(szName, "RotY")) {
		*rgDispId = 1023;
	}
	else if (!lstrcmpi(szName, "RotAndTra2")) {
		*rgDispId = 1024;
	}
	else if (!lstrcmpi(szName, "RotZ")) {
		*rgDispId = 1025;
	}
	else if (!lstrcmpi(szName, "RotAndTra3")) {
		*rgDispId = 1026;
	}
	else if (!lstrcmpi(szName, "TransX")) {
		*rgDispId = 1027;
	}
	else if (!lstrcmpi(szName, "RotAndTra4")) {
		*rgDispId = 1028;
	}
	else if (!lstrcmpi(szName, "TransY")) {
		*rgDispId = 1029;
	}
	else if (!lstrcmpi(szName, "RotAndTra5")) {
		*rgDispId = 1030;
	}
	else if (!lstrcmpi(szName, "TransZ")) {
		*rgDispId = 1031;
	}
	else if (!lstrcmpi(szName, "RotAndTra6")) {
		*rgDispId = 1032;
	}
	else if (!lstrcmpi(szName, "ObjRotX")) {
		*rgDispId = 1033;
	}
	else if (!lstrcmpi(szName, "RotAndTra7")) {
		*rgDispId = 1034;
	}
	else if (!lstrcmpi(szName, "ObjRotY")) {
		*rgDispId = 1035;
	}
	else if (!lstrcmpi(szName, "RotAndTra8")) {
		*rgDispId = 1036;
	}
	else if (!lstrcmpi(szName, "ObjRotZ")) {
		*rgDispId = 1037;
	}
	else if (!lstrcmpi(szName, "EdgeFactorUI")) {
		*rgDispId = 1038;
	}
	else if (!lstrcmpi(szName, "CollisionReductionFactor")) {
		*rgDispId = 1039;
	}
	else if (!lstrcmpi(szName, "Name")) {
		*rgDispId = 1040;
	}
	else if (!lstrcmpi(szName, "EnableStaticRendering")) {
		*rgDispId = 1041;
	}
	else if (!lstrcmpi(szName, "Elasticity")) {
		*rgDispId = 1042;
	...
	else if (!lstrcmpi(szName, "IsToy")) {
		*rgDispId = 1045;
	}
	else if (!lstrcmpi(szName, "BackfacesEnabled")) {
		*rgDispId = 1046;
	}
	else if (!lstrcmpi(szName, "HasHitEvent")) {
		*rgDispId = 1047;
	...
	else if (!lstrcmpi(szName, "Opacity")) {
		*rgDispId = 1055;
	}
	else if (!lstrcmpi(szName, "AddBlend")) {
		*rgDispId = 1056;
	}
	else if (!lstrcmpi(szName, "Color")) {
		*rgDispId = 1057;
	...
	else if (!lstrcmpi(szName, "HitThreshold")) {
		*rgDispId = 1061;
	}
	else if (!lstrcmpi(szName, "ObjectSpaceNormalMap")) {
		*rgDispId = 1062;
	}
	else {
		return DISP_E_UNKNOWNNAME;
...
STDMETHODIMP HitTarget::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	char szName[MAXSTRING];
	WideCharToMultiByteNull(CP_ACP, 0, *rgszNames, -1, szName, MAXSTRING, nullptr, nullptr);
	if (!lstrcmpi(szName, "TimerEnabled")) {
		*rgDispId = 1000;
	}
	else if (!lstrcmpi(szName, "TimerInterval")) {
		*rgDispId = 1001;
	...
	else if (!lstrcmpi(szName, "DropSpeed")) {
		*rgDispId = 1026;
	}
	else if (!lstrcmpi(szName, "IsDropped")) {
		*rgDispId = 1027;
	...
	else if (!lstrcmpi(szName, "LegacyMode")) {
		*rgDispId = 1029;
	}
	else if (!lstrcmpi(szName, "RaiseDelay")) {
		*rgDispId = 1030;
	}
	else if (!lstrcmpi(szName, "PhysicsMaterial")) {
		*rgDispId = 1031;
...
STDMETHODIMP Gate::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	...
	else if (!lstrcmpi(szName, "Open")) {
		*rgDispId = 1008;
	}
	else if (!lstrcmpi(szName, "Damping")) {
		*rgDispId = 1009;
	}
	else if (!lstrcmpi(szName, "GravityFactor")) {
		*rgDispId = 1010;
	}
	else if (!lstrcmpi(szName, "Move")) {
		*rgDispId = 1011;
	}
	else if (!lstrcmpi(szName, "Material")) {
		*rgDispId = 1012;
	...
	else if (!lstrcmpi(szName, "CloseAngle")) {
		*rgDispId = 1016;
	}
	else if (!lstrcmpi(szName, "OpenAngle")) {
		*rgDispId = 1017;
	}
	else if (!lstrcmpi(szName, "Collidable")) {
		*rgDispId = 1018;
	...
	else if (!lstrcmpi(szName, "TwoWay")) {
		*rgDispId = 1021;
	}
	else if (!lstrcmpi(szName, "ShowBracket")) {
		*rgDispId = 1022;
	}
	else if (!lstrcmpi(szName, "CurrentAngle")) {
		*rgDispId = 1023;
...
STDMETHODIMP Spinner::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	...
	else if (!lstrcmpi(szName, "AngleMax")) {
		*rgDispId = 1014;
	}
	else if (!lstrcmpi(szName, "AngleMin")) {
		*rgDispId = 1015;
	}
	else if (!lstrcmpi(szName, "Elasticity")) {
		*rgDispId = 1016;
...
STDMETHODIMP Ramp::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	...
	else if (!lstrcmpi(szName, "WidthBottom")) {
		*rgDispId = 1005;
	}
	else if (!lstrcmpi(szName, "WidthTop")) {
		*rgDispId = 1006;
	}
	else if (!lstrcmpi(szName, "Material")) {
		*rgDispId = 1007;
	...
	else if (!lstrcmpi(szName, "HasWallImage")) {
		*rgDispId = 1011;
	}
	else if (!lstrcmpi(szName, "LeftWallHeight")) {
		*rgDispId = 1012;
	}
	else if (!lstrcmpi(szName, "RightWallHeight")) {
		*rgDispId = 1013;
	}
	else if (!lstrcmpi(szName, "UserValue")) {
		*rgDispId = 1014;
	}
	else if (!lstrcmpi(szName, "VisibleLeftWallHeight")) {
		*rgDispId = 1015;
	}
	else if (!lstrcmpi(szName, "VisibleRightWallHeight")) {
		*rgDispId = 1016;
	}
	else if (!lstrcmpi(szName, "Elasticity")) {
		*rgDispId = 1017;
	...
	else if (!lstrcmpi(szName, "WireDiameter")) {
		*rgDispId = 1025;
	}
	else if (!lstrcmpi(szName, "WireDistanceX")) {
		*rgDispId = 1026;
	}
	else if (!lstrcmpi(szName, "WireDistanceY")) {
		*rgDispId = 1027;
	}
	else if (!lstrcmpi(szName, "ReflectionEnabled")) {
		*rgDispId = 1028;
...
STDMETHODIMP Flasher::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	...
	else if (!lstrcmpi(szName, "ImageA")) {
		*rgDispId = 1009;
	}
	else if (!lstrcmpi(szName, "ImageB")) {
		*rgDispId = 1010;
	}
	else if (!lstrcmpi(szName, "DisplayTexture")) {
		*rgDispId = 1011;
	...
	else if (!lstrcmpi(szName, "ModulateVsAdd")) {
		*rgDispId = 1014;
	}
	else if (!lstrcmpi(szName, "UserValue")) {
		*rgDispId = 1015;
	...
	else if (!lstrcmpi(szName, "VideoCapWidth")) {
		*rgDispId = 1019;
	}
	else if (!lstrcmpi(szName, "VideoCapHeight")) {
		*rgDispId = 1020;
	}
	else if (!lstrcmpi(szName, "VideoCapUpdate")) {
		*rgDispId = 1021;
	}
	else if (!lstrcmpi(szName, "DepthBias")) {
		*rgDispId = 1022;
	}
	else if (!lstrcmpi(szName, "Filter")) {
		*rgDispId = 1023;
	}
	else if (!lstrcmpi(szName, "Amount")) {
		*rgDispId = 1024;
	}
	else if (!lstrcmpi(szName, "TimerEnabled")) {
		*rgDispId = 1025;
...
STDMETHODIMP Rubber::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	...
	else if (!lstrcmpi(szName, "Thickness")) {
		*rgDispId = 1004;
	}
	else if (!lstrcmpi(szName, "RotX")) {
		*rgDispId = 1005;
	...
	else if (!lstrcmpi(szName, "EnableShowInEditor")) {
		*rgDispId = 1017;
	}
	else if (!lstrcmpi(szName, "ReflectionEnabled")) {
		*rgDispId = 1018;
...
STDMETHODIMP BallEx::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	...
	else if (!lstrcmpi(szName, "VelX")) {
		*rgDispId = 1002;
	}
	else if (!lstrcmpi(szName, "VelY")) {
		*rgDispId = 1003;
	}
	else if (!lstrcmpi(szName, "Z")) {
		*rgDispId = 1004;
	}
	else if (!lstrcmpi(szName, "VelZ")) {
		*rgDispId = 1005;
	}
	else if (!lstrcmpi(szName, "AngVelX")) {
		*rgDispId = 1006;
	}
	else if (!lstrcmpi(szName, "AngVelY")) {
		*rgDispId = 1007;
	}
	else if (!lstrcmpi(szName, "AngVelZ")) {
		*rgDispId = 1008;
	}
	else if (!lstrcmpi(szName, "AngMomX")) {
		*rgDispId = 1009;
	}
	else if (!lstrcmpi(szName, "AngMomY")) {
		*rgDispId = 1010;
	}
	else if (!lstrcmpi(szName, "AngMomZ")) {
		*rgDispId = 1011;
	}
	else if (!lstrcmpi(szName, "Color")) {
		*rgDispId = 1012;
	...
	else if (!lstrcmpi(szName, "FrontDecal")) {
		*rgDispId = 1014;
	}
	else if (!lstrcmpi(szName, "DecalMode")) {
		*rgDispId = 1015;
	}
	else if (!lstrcmpi(szName, "UserValue")) {
		*rgDispId = 1016;
	...
	else if (!lstrcmpi(szName, "ID")) {
		*rgDispId = 1019;
	}
	else if (!lstrcmpi(szName, "Name")) {
		*rgDispId = 1020;
	}
	else if (!lstrcmpi(szName, "BulbIntensityScale")) {
		*rgDispId = 1021;
	}
	else if (!lstrcmpi(szName, "ReflectionEnabled")) {
		*rgDispId = 1022;
	}
	else if (!lstrcmpi(szName, "PlayfieldReflectionScale")) {
		*rgDispId = 1023;
	}
	else if (!lstrcmpi(szName, "ForceReflection")) {
		*rgDispId = 1024;
	}
	else if (!lstrcmpi(szName, "Visible")) {
		*rgDispId = 1025;
...
STDMETHODIMP DispReel::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	...
	else if (!lstrcmpi(szName, "Spacing")) {
		*rgDispId = 1005;
	}
	else if (!lstrcmpi(szName, "IsTransparent")) {
		*rgDispId = 1006;
	}
	else if (!lstrcmpi(szName, "Sound")) {
		*rgDispId = 1007;
	}
	else if (!lstrcmpi(szName, "Steps")) {
		*rgDispId = 1008;
	}
	else if (!lstrcmpi(szName, "TimerEnabled")) {
		*rgDispId = 1009;
	...
	else if (!lstrcmpi(szName, "Range")) {
		*rgDispId = 1013;
	}
	else if (!lstrcmpi(szName, "Name")) {
		*rgDispId = 1014;
	}
	else if (!lstrcmpi(szName, "UpdateInterval")) {
		*rgDispId = 1015;
	}
	else if (!lstrcmpi(szName, "UserValue")) {
		*rgDispId = 1016;
	}
	else if (!lstrcmpi(szName, "UseImageGrid")) {
		*rgDispId = 1017;
	}
	else if (!lstrcmpi(szName, "Visible")) {
		*rgDispId = 1018;
	}
	else if (!lstrcmpi(szName, "ImagesPerGridRow")) {
		*rgDispId = 1019;
	}
	else if (!lstrcmpi(szName, "AddValue")) {
		*rgDispId = 1020;
	}
	else if (!lstrcmpi(szName, "ResetToZero")) {
		*rgDispId = 1021;
	}
	else if (!lstrcmpi(szName, "SpinReel")) {
		*rgDispId = 1022;
	}
	else if (!lstrcmpi(szName, "SetValue")) {
		*rgDispId = 1023;
	}
	else {
		return DISP_E_UNKNOWNNAME;
...
STDMETHODIMP LightSeq::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	...
	else if (!lstrcmpi(szName, "Collection")) {
		*rgDispId = 1001;
	}
	else if (!lstrcmpi(szName, "CenterX")) {
		*rgDispId = 1002;
	}
	else if (!lstrcmpi(szName, "CenterY")) {
		*rgDispId = 1003;
	}
	else if (!lstrcmpi(szName, "UpdateInterval")) {
		*rgDispId = 1004;
	...
	else if (!lstrcmpi(szName, "Play")) {
		*rgDispId = 1008;
	}
	else if (!lstrcmpi(szName, "StopPlay")) {
		*rgDispId = 1009;
	}
	else {
		return DISP_E_UNKNOWNNAME;
